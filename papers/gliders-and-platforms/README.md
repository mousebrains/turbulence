...
| Merckelbach, Berger, Krahmann, Dengler & Carpenter (2019), "A dynamic flight model for Slocum gliders and implications for turbulence microstructure measurements," *JTECH* **36**, 281–296. [doi:10.1175/JTECH-D-18-0168.1](https://doi.org/10.1175/JTECH-D-18-0168.1) | `Merckelbach_2019_Dynamic_Flight_Model_Slocum.pdf` | **The first published `U_EM` scale factor.** Calibrates the flight model in situ against a DVL (glider COMET) and against an AEM1-G on a MicroRider (glider IFM03, Peru, 2017). The vertical velocity from U_EMC and pitch was "consistently larger in magnitude than the measured depth rate", so U_EMC was scaled by **0.93**. That factor used the AOA from the steady-state model with COMET's coefficients, so it holds only as far as that AOA does. Rockland found "a similar scaling factor" on a SeaExplorer carrying an ADCP (Lueck 2018, pers. comm.). Also: the EM reads the body-axis component, U = U_EMC / cos α (eq. 13); JFE calibrated the sensor by towing it "mounted to a MicroRider hull"; final a = 7.5 rad⁻¹ and C_D1 = 10.5 rad⁻², with C_D0 = 0.147 from the parabola fit at that C_D1 (the joint IFM03 fit gave 0.136 with a = 7.7; S = 0.1 m²). The calibrated model removes a ~1 cm s⁻¹ speed bias, "roughly a factor of 1.2" in ε. **Not to be confused with the 0.83** applied to COMET's *pitch and roll*, which came from a tilt sensor that had leaked electrolyte. Data: [doi:10.5281/zenodo.2270123](https://doi.org/10.5281/zenodo.2270123); code (`gliderflight`): [doi:10.5281/zenodo.2222694](https://doi.org/10.5281/zenodo.2222694). |
| Tanaka, Hasegawa, Okunishi, Yasuda & Welch (2022), "In situ calibration of underwater glider flight model using acoustic Doppler current profilers," *JTECH* **39**, 1331–1352. [doi:10.1175/JTECH-D-21-0074.1](https://doi.org/10.1175/JTECH-D-21-0074.1) | `Tanaka_2022_Glider_Flight_Model_ADCP_Calibration.pdf` | **The `U_EM` over-read measured against an ADCP, and the published statement that pressure and pitch alone cannot separate it from the flight coefficients.** The Doppler profiler's nearest bin gives the AOA directly: a mean of −4.4° at a mean −23.7° pitch on Slocum descents at 0.28 m s⁻¹, binned over roughly 21–27°. C_D0 and C_D1 come from the observed AOA–C_D relation and the lift coefficient a from the pitch–AOA relation: Slocum C_D0 = 0.18, C_D1 = 5.92 rad⁻², a = 5.4 rad⁻¹; SeaExplorer (a first) C_D0 = 0.19, C_D1 = 5.0 rad⁻², a = 4.0 rad⁻¹. Using the 2010 (2019) coefficients instead shifts ε by 7% (17%) on average. **Appendix E is the part to read.** v_ADCP/v_EM peaks at **0.85** on the Slocum (AEM1-GA on a MicroRider) and **0.90** on the SeaExplorer. The 2019 recipe (match sin(θ+α)/cos α to w/v_EM using the AOA modeled with the 2019 coefficients, C_D0 = 0.164 on these data) gives **0.90** on the same Slocum descents, so a factor from that recipe absorbs the AOA model's error. The paper says AOA "is highly sensitive to the scaling factor", and that coefficients and scale factor "cannot be determined uniquely without having another source of velocity measurement". "Our MR data" give ~0.91 on ascent against ~0.85 on descent, for reasons "not identified yet". The paper does not say which glider or which reference produced the ascent value, and for the Slocum it cannot be a direct ADCP ratio: that ADCP did not record on ascent. It closes by asking for a tow-tank calibration with the EM still attached to the MR/glider. |
| Merckelbach & Carpenter (2021), "Ocean glider flight in the presence of surface waves," *JTECH* **38**, 1265–1275. [doi:10.1175/JTECH-D-20-0206.1](https://doi.org/10.1175/JTECH-D-20-0206.1) | `Merckelbach_Carpenter_2021_Glider_Flight_Surface_Waves.pdf` | Extends the flight model to surface waves. Wave-induced relative velocity costs ε "generally less than 10%" near the surface. For **shallow- and intermediate-depth waves**, though (not deep-water waves), the wave pressure perturbation corrupts the glider's measured pressure, and with it the dP/dt that `speed.method: flight` and `pressure` both rest on. |
...
- **The published EM-on-MicroRider over-reads are 7.5–17.6%**: a factor f means
  U_EM reads 1/f − 1 high, for f = 0.93–0.85 (plus Rockland's unpublished
  SeaExplorer case). The flight-vs-`U_EM`
...
- **A scale factor derived from pressure and pitch is only as good as the AOA
  ...
  unknowns. Tanaka's Appendix E states the published version (coefficients and
  scale factor "cannot be determined uniquely without having another source of
  velocity measurement"), and their 0.85 (direct, against the ADCP) versus 0.90
  (via the 2019 AOA) on the same descents shows its size. An independent
  velocity separates them. That a spread of pitch angles also would, through
  sin()'s curvature, is this repository's reasoning, not a published result.
- **Both papers fit a gain only.** A zero offset c would instead give a factor
  ...
  the AEM1-G work, not a published result, and a weak one: the ascent value's
  reference is not stated (above).
- **The steady-glide AOA falls with pitch**, so the `aoa_deg: 3.0` default
  cannot be right at every attitude. Solving
  a·α·tan(θ + α) = C_D0 + C_D1·α² with Tanaka's Slocum coefficients gives 4.25°
  at 23.7° against their measured 4.4°. That agreement holds by construction,
  since a was fitted to those data. The same coefficients give 3.2° at 30°,
  2.2° at 40° and 1.9° at 45°.
  - The 2019 coefficients give 1.8–2.0°, 1.2–1.3° and 1.0–1.1° at the same
    pitches with Merckelbach's C_D0 of 0.136–0.147.
  - With C_D0 = 0.164, the value used on Tanaka's Slocum, they give 2.2°, 1.5°
    and 1.2°.
  - C_D0 is glider-specific.
  - The only directly measured Slocum AOA in this group is Tanaka's, over about
    21–27° pitch; anything steeper is model extrapolation.
