...
class TestEpsilonConfigDefaultsResolved:
    """process_file resolves epsilon/chi via merge_config THEN
    resolve_window_config, so an omitted or null fft setting uses the
    documented duration default (fft_sec 1.0, resolved at the file's
    sampling rate) -- never _compute_epsilon's own 1024-sample default --
    and diss_length is always a concrete integer (no 4*None TypeError;
    M-8/M-9 lineage)."""

    def test_omitted_and_null_fft_settings_resolve_to_default(self):
        from odas_tpw.perturb.config import merge_config, resolve_window_config

        for raw in ({}, {"fft_length": None}, {"fft_sec": None}):
            eps = resolve_window_config(merge_config("epsilon", raw), 512.0)
            assert eps["fft_length"] == 512  # 1 s at 512 Hz
            assert eps["diss_length"] == 2048

    def test_windows_scale_with_sampling_rate(self):
        from odas_tpw.perturb.config import merge_config, resolve_window_config

        # The same (default) config sizes windows per instrument rate.
        for fs, want in ((512.0, 512), (1024.0, 1024), (2048.0, 2048)):
            eps = resolve_window_config(merge_config("epsilon", {}), fs)
            assert (eps["fft_length"], eps["diss_length"]) == (want, 4 * want)
