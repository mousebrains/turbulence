...
from microstructure_tpw.perturb.config import merge_config, resolve_output_dir
...
def process_file(
    ...
    """
    from microstructure_tpw.perturb.ct_align import ct_align
    from microstructure_tpw.perturb.epsilon_combine import mk_epsilon_mean
    from microstructure_tpw.perturb.fp07_cal import fp07_calibrate
    ...
    if hotel_data is not None:
        from microstructure_tpw.perturb.hotel import interpolate_hotel
    ...
    if ctd_cfg.get("enable", True) and "ctd" in output_dirs:
        try:
            from microstructure_tpw.perturb.ctd import ctd_bin_file
...
def run_merge(config: dict) -> list[Path]:
    """Merge split .p files."""
    from microstructure_tpw.perturb.discover import find_p_files
    from microstructure_tpw.perturb.merge import find_mergeable_files, merge_p_files
...
def run_pipeline(config: dict, p_files: list[Path] | None = None) -> None:
    ...
    """
    from microstructure_tpw.perturb.discover import find_p_files
    from microstructure_tpw.perturb.gps import create_gps
    ...
    if hotel_cfg.get("enable", False) and hotel_cfg.get("file"):
        from microstructure_tpw.perturb.hotel import load_hotel
