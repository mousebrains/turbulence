...
from microstructure_tpw.rsi.viewer_base import (
    ProfileViewer,
...
class DissLookViewer(ProfileViewer):
    ...
    def _draw_chi_profile(self):
        ...
        for i, (name, _) in enumerate(self.therm_fast):
            ...
            if np.any(valid):
                ax.plot(chi_mean[valid], P[valid], f"{c}o-",
    ...
    def _draw_fm_profile(self):
        ...
        for i, (name, _) in enumerate(self.shear):
            ...
            if np.any(valid):
                ...
                ax.plot(fm_valid, P[valid], f"{c}o-",
                        markersize=3, linewidth=0.8, label=name)
                has_data = True

        if has_data:
            ax.axvline(
                1.0, color="k", linestyle="-", linewidth=1.0, alpha=0.7,
                label=f"FM>1: {', '.join(pct1_lines)}",
            )
            ax.axvline(
                1.15, color="0.5", linestyle="--", linewidth=0.7, alpha=0.5,
            ...
            ax.text(0.5, 0.5, "No valid FM", transform=ax.transAxes,
                    ha="center", va="center")
    ...
    def _draw_chi_spectra(self, sel_spec):
        ...
        if not r["chi_obs_specs"]:
            ax.text(0.5, 0.5, "No \u03c7 spectra", transform=ax.transAxes,
                    ha="center", va="center")
            return
        ...
        for i, (name, _) in enumerate(self.therm_fast):
            ...
            if np.any(valid):
                ax.loglog(K_chi[valid], obs[valid], c,
                          linewidth=0.8, alpha=0.6, label=name)
            ...
            if np.any(valid_m1) and np.isfinite(chi_m1):
                ax.loglog(
                    K_chi[valid_m1], m1_spec[valid_m1], c,
            ...
            if np.any(valid_m2) and np.isfinite(chi_m2):
                ax.loglog(
                    K_chi[valid_m2], m2_spec[valid_m2], c,
                    linewidth=1.2, linestyle="-.", alpha=0.9,
