...
class L2Params:
    ...
    speed_tau: float  # speed smoothing time constant [s] (ODAS default 1.5)
...
def _read_l2_params(ds: netCDF4.Dataset, l1: L1Data) -> L2Params:
    ...
    Parameters are read from the L2_cleaned group attributes first, then
    from global attributes (e.g. ``speed_cutout``), then defaults.
    """
    g = ds.groups["L2_cleaned"]
    attrs = {a: g.getncattr(a) for a in g.ncattrs()}
    gattrs = {a: ds.getncattr(a) for a in ds.ncattrs()}

    # Defaults per Lueck et al. (2024) recommendations.
    # HP_cut from the global attrs has a different meaning (e.g. 0 for
