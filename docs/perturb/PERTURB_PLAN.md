...
src/odas_tpw/perturb/
    ...
    binning.py            # Depth/time binning engine for profiles, diss, and chi
...
| Function | Used for |
|----------|----------|
| `gsw.SP_from_C` | Practical salinity from conductivity |
| `gsw.SA_from_SP` | Absolute salinity |
| `gsw.CT_from_t` | Conservative temperature |
| `gsw.sigma0` | Potential density anomaly |
| `gsw.rho` | In-situ density |
| `gsw.Nsquared` | Buoyancy frequency N² |
| `gsw.z_from_p` | Depth from pressure |

**Note:** Use `gsw` directly for all seawater properties in `seawater.py` (may optimize later).
...
perturb diss config.yaml [VMP/*.p]               # dissipation (from profile NetCDFs)
perturb chi config.yaml                          # compute chi from diss results (Method 1)
perturb ctd config.yaml [VMP/*.p]                # CTD time-binning with GPS
perturb bin config.yaml                          # bin profiles, diss, and chi by depth/time
...
The `run` subcommand executes: trim → merge → profiles → diss → chi (if enabled) → ctd → bin → combo sequentially.
...
fp07:
  ...
  max_lag_seconds: 10
  reference: "JAC_T"
  must_be_negative: true   # for falling VMPs, expect negative lag
...
epsilon:
  ...
  goodman: true
  f_AA: 98.0
  f_limit: null            # max frequency for diss estimation
  fit_order: 3
  despike_thresh: 8
  despike_smooth: 0.5
  salinity: null
...
ctd:
  ...
  T_name: "JAC_T"
  C_name: "JAC_C"
  variables: null          # which channels to bin (null = auto-detect)
...
binning:
  ...
  chi_width: null           # override bin width for chi (default: same as diss_width)
  chi_aggregation: null     # override aggregation for chi (default: same as diss_aggregation)
...
output_root/
  ...
  profiles_00/                     # per-profile NetCDF
    ...
    ...
  diss_00/                         # per-profile dissipation (epsilon only)
    SN479_0001_prof01.nc           #   epsilon (per-probe + epsilonMean)
    SN479_0001_prof02.nc
    ...
  chi_00/                          # per-profile chi (if chi.enable)
    SN479_0001_prof01.nc           #   chi (per-probe), kB, fom, K_max_ratio
    SN479_0001_prof02.nc
    ...
  ...
  profiles_binned_00/              # per-file binned profiles
    ...
    SN479_0002.nc
    ...
  diss_binned_00/                  # per-file binned diss
    SN479_0001.nc
    SN479_0002.nc
    ...
  chi_binned_00/                   # per-file binned chi (if chi.enable)
    SN479_0001.nc
    SN479_0002.nc
    ...
  profiles_combo_00/               # combined profiles (all files merged)
    combo.nc                       #   2D: bins x profiles
  diss_combo_00/                   # combined diss (all files merged)
    combo.nc
  chi_combo_00/                    # combined chi (all files merged, if chi.enable)
    combo.nc
...
### Dissipation NetCDF contents (`diss_00/`)
- Dim: `time` (dissipation window centers)
- Epsilon vars (per-probe): `e_1`, `e_2`, `K_max_1`, `K_max_2`, `FM_1`, `FM_2`, `mad_1`, `mad_2`, `method_1`, `method_2`
- Combined: `epsilonMean`, `epsilonLnSigma` (from mk_epsilon_mean)
- Common: `speed`, `nu`, `P_mean`, `T_mean`
- Spectra: `K`, `F`, `spec_shear_1`, `spec_shear_2`, `spec_nasmyth_1`, `spec_nasmyth_2`
- Attrs: parameters used, source profile path

### Chi NetCDF contents (`chi_00/`)
- Dim: `time` (chi window centers)
- Chi vars (per-probe): `chi_1`, `chi_2`, `kB_1`, `kB_2`, `K_max_T_1`, `K_max_T_2`, `fom_T_1`, `fom_T_2`, `K_max_ratio_T_1`, `K_max_ratio_T_2`
- Common: `speed`, `nu`, `P_mean`, `T_mean`
- Spectra: `K`, `spec_gradT_1`, `spec_gradT_2`, `spec_batch_1`, `spec_batch_2`
- Attrs: parameters used, source profile path, source diss path
...
### Binned NetCDF contents (`profiles_binned_00/`, `diss_binned_00/`, `chi_binned_00/`)
- Dims: `bin` (depth or time bins), `profile`
- For profiles: all profile variables aggregated into bins
- For diss: epsilon, epsilonMean, QC metrics aggregated into bins
- For chi: chi, kB, QC metrics aggregated into bins
...
- Steps:
  ...
  4. If `must_be_negative`: restrict lag search to negative values (falling VMPs)
  5. Compute median lag across profiles
...
- Parameters: `depth_window`, `depth_minimum`, `speed_factor`, `median_factor`, `vibration_frequency`, `vibration_factor`
...
- Find depth where std drops below `quantile` threshold of median
...
- Steps:
  ...
  2. Time-bin at configured interval (default 0.5s)
...
- Uses BOTH up and down portions of the cast (full file data, not profile-segmented)
...
- `bin_by_depth(profiles: list[Path], bin_width: float, aggregation: str) -> xr.Dataset`
  - Loads per-profile NetCDFs, bins each by depth, assembles into 2D (bin x profile)
...
- `bin_diss(diss_files: list[Path], bin_width: float, aggregation: str, method: str) -> xr.Dataset`
  - Bins dissipation estimates by depth or time
  - Handles per-probe variables (e.g., `e_1`, `e_2`) and combined variables (`epsilonMean`)
- `bin_chi(chi_files: list[Path], bin_width: float, aggregation: str, method: str) -> xr.Dataset`
  - Bins chi estimates by depth or time
  - Handles per-probe variables (e.g., `chi_1`, `chi_2`)
  - Uses `chi_width`/`chi_aggregation` overrides from config if set, otherwise falls back to `diss_width`/`diss_aggregation`
- Helper: `_bin_array(values, coords, bin_edges, agg_func) -> ndarray`
- Writes per-file binned NetCDF to `profiles_binned_NN/`, `diss_binned_NN/`, and `chi_binned_NN/`
...
- `CHI_SCHEMA` dict — variable definitions for chi combo NetCDF
...
- `process_file(p_path, config, gps, output_dirs) -> dict`
  - Per-file: PFile → profiles → FP07 cal → CT align → GPS → seawater → bottom → top trim → write profile NetCDFs → per-profile diss → write diss NetCDFs → CTD binning
  - If `chi.enable`: per-profile chi (reading diss output) → write chi NetCDFs
...
- Stage runners (called by individual subcommands):
  ...
  - `run_chi(config) -> list[Path]` — reads diss results, computes chi via Method 1
  - `run_ctd(config) -> list[Path]`
  - `run_bin(config) -> list[Path]` — bins profiles, diss, and chi (if enabled)
  - `run_combo(config) -> list[Path]` — combos profiles, diss, chi (if enabled), and CTD
- `run_pipeline(config) -> None`
  - Full: discover → trim → merge → parallel process_file → bin → combo

### `cli.py` — Subcommand CLI
- `main()` — argparse with subcommands: `init`, `run`, `trim`, `merge`, `profiles`, `diss`, `chi`, `ctd`, `bin`, `combo`
...
After loading PFile, the pipeline forks into two independent paths:
- **CTD fork:** time-bins scalars (T, C, Chlorophyll, DO, etc.) using BOTH up and down portions of the cast (full file data, not profile-segmented)
- **Profile fork:** extracts profiles → diss → chi (uses detected profiling segments only)

### Per-file pipeline (parallelized)

```
PFile(path)
  |
  +---> [CTD fork] CTD time-binning (full file, both up+down) --> ctd_NN/
  |
  +---> [Profile fork]
        ...
        --> per-profile:
            ...
            --> write diss NetCDF --> diss_NN/
        --> if chi.enable:
            --> per-profile:
                --> get_chi(epsilon_ds=diss) (Method 1, spectral fitting)
                --> write chi NetCDF --> chi_NN/
```
...
--> bin diss by depth/time --> diss_binned_NN/
--> if chi.enable: bin chi by depth/time --> chi_binned_NN/
--> combo: merge all binned profiles --> profiles_combo_NN/combo.nc
--> combo: merge all binned diss --> diss_combo_NN/combo.nc
--> if chi.enable: combo: merge all binned chi --> chi_combo_NN/combo.nc
...
13. **`binning.py`** — depth/time binning engine (profiles, diss, chi)
14. **`netcdf_schema.py`** — CF/ACDD schema definitions
15. **`combo.py`** — combined NetCDF assembly
16. **`pipeline.py`** — per-file orchestration + stage runners (including `run_chi`)
17. **`cli.py`** — subcommand CLI (including `chi` subcommand)
...
5. Diss NetCDFs contain per-probe epsilon (`e_1`, `e_2`), `epsilonMean` — no chi
6. Chi NetCDFs (if enabled) contain per-probe chi, kB, fom, K_max_ratio — separate from diss
7. CTD NetCDFs contain time-binned scalars with GPS and seawater properties
...
15. Chi config retains spectral parameters + `enable` flag controls activation
16. Run individual subcommands (`perturb profiles`, `perturb diss`, `perturb chi`, `perturb bin`, `perturb combo`) independently — stages work in isolation
