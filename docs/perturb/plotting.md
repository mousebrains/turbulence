...
| `section`  | Depth-vs-x scalar sections (T / S / density / ...) from the CTD combo, with selectable x-axis. |
...
| `--out-dir DIR` | Output directory for `section_<name>.png` (default: `--root`). |
...
```bash
# Quick time section of the default scalars
perturb-plot section --root ~/Desktop/vmp_results

# A latitude section of temperature only, top 150 m
perturb-plot section --root RESULTS --xaxis latitude --var JAC_T --depth-max 150

# Multiple named sections from a file
perturb-plot section --root RESULTS --sections sections.yaml --out-dir figs/
```
