...
| `microstructure_sensors/AEM1-G_datasheet_JFE_2018-06.pdf` + its `.gitignore` exception | 1 file | **commit here** | Vendor datasheet for a sensor we fly, filed beside `shear_sensitivities.csv`. Cited by `em_bench_zero.py`'s header for the ±0.5 cm/s or ±2 %RD spec. Vendor datasheets are not per-unit calibration sheets and are worth tracking. |
