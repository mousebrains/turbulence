...
This directory contains MATLAB scripts that support the `microstructure_tpw.rsi` package. The files fall into three categories:

1. **Python-mirrored implementations** — MATLAB functions that implement the same algorithms as modules in `src/microstructure_tpw/rsi/`. These must stay self-consistent: if the Python implementation changes, the corresponding MATLAB function should be updated (or vice versa) so that both produce equivalent results.
...
These files are standalone MATLAB reimplementations of algorithms in `src/microstructure_tpw/rsi/`. They share the same formulas, constants, and default parameters. When editing the Python side, check the corresponding MATLAB file for consistency (and vice versa).
