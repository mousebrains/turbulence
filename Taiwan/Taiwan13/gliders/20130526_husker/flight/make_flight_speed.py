...
OUT = HERE / "husker_flight_hotel.nc"
...
def load():
    ...
    h = xr.open_dataset(HERE.parent / "hotel" / "hotel.nc")
    th = epoch(h["sci_ctd41cp_timestamp"].values)
    gl = np.isfinite(th) & np.isfinite(h["lat"].values) & np.isfinite(h["lon"].values)
    lat = np.interp(t, th[gl], h["lat"].values[gl])
    lon = np.interp(t, th[gl], h["lon"].values[gl])
    p_dbar = P * 10.0
