...
# ]
# ///
"""Through-water speed for MicroRider MR046 on glider husker -- Merckelbach flight model.

    uv run make_flight_speed.py calibrate   # -> calibration.json   (resumable, ~30 min)
    uv run make_flight_speed.py run         # -> husker_flight_hotel.nc (perturb hotel file)
...
INPUTS  flight_inputs.nc (dinkum-hotel, flight clock), mr_clock_model.json
        (mr_clock.py), ../hotel/hotel.nc (drift-corrected lat/lon, for density).
...
def load():
    ...
    lat = np.full(t.shape, 21.6)   # jane worked 21-22 N, 119-120 E; density is
    lon = np.full(t.shape, 119.8)  # insensitive to this at ~1e-3 kg/m3
    p_dbar = P * 10.0
...
def lag_model(t):
    if not (HERE / "mr_clock_model.json").exists():
        return np.zeros(np.shape(t))   # jane carried no MicroRider
...
def run():
    ...
    t_mr = d["t"]  # jane carried no MicroRider: stay on the glider clock
    ...
    ds = xr.Dataset(
        {
            ...
            "glider_time": var(d["t"], "seconds since 1970-01-01T00:00:00+00:00", "husker flight-computer time (m_present_time)"),
        ...
        coords={"time": ("time", t_mr[sel], dict(units="seconds since 1970-01-01T00:00:00+00:00", calendar="standard",
                                                   ...
                                                   long_name="time on the MR046 CLOCK (glider time minus mr_clock_lag)"))},
        attrs=dict(
            title="Glider husker flight-model hotel file for MicroRider MR046, Taiwan 2013",
            Conventions="CF-1.13",
